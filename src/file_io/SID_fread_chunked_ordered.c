...
size_t SID_fread_chunked_ordered(void *buffer, size_t n_x_read_local, SID_fp *fp) {
    ...
    for(i_rank = 0; i_rank < SID.n_proc; i_rank++) {
        ...
        SID_Bcast(&n_x_read_local_bcast, 1, SID_SIZE_T, i_rank, SID_COMM_WORLD);
    ...
    for(i_chunk = 0, i_x_read_chunk = 0, i_x_chunk = i_x_offset_local + i_x_offset_global; i_chunk < fp->chunked_header.n_chunk; i_chunk++) {
        if(fp->i_x_start_chunk[i_chunk] <= i_x_chunk && fp->i_x_last_chunk[i_chunk] >= i_x_chunk) {
            n_skip    = i_x_chunk - fp->i_x_start_chunk[i_chunk];
            n_x_chunk = GBP_MIN(n_x_read_local - i_x_read_chunk, fp->i_x_step_chunk[i_chunk] - n_skip);
        } else {
            n_x_chunk = 0;
            n_skip    = 0;
        }
            ...
            for(i_group = 0; i_group < SID.n_proc; i_group++) {
                if(SID.My_group == i_group && n_x_chunk > 0) {
                    ...
                    if(n_x_chunk > 0) {
                        ...
                        SID_fread((char *)buffer + i_x_read_chunk * fp->chunked_header.item_size, fp->chunked_header.item_size, n_x_chunk, fp);
                        i_x_chunk += n_x_chunk;
