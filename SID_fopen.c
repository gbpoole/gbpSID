...
int SID_fopen(char   *filename,
   ...
   MPI_Bcast(&filename_test_hack_length,1,MPI_INT,MASTER_RANK,MPI_COMM_WORLD);
   filename_test_hack=(char *)malloc(sizeof(char)*filename_test_hack_length);
   if(SID.I_am_Master)
     strcpy(filename_test_hack,filename);
   MPI_Bcast(filename_test_hack,filename_test_hack_length,MPI_CHAR,MASTER_RANK,MPI_COMM_WORLD);
   if(!strcmp(mode,"w")){
   ...
   // -------------------------------------------
   if(!strcmp(mode,"r")){
     r_val=!(MPI_File_open(MPI_COMM_WORLD,
                           filename,
                           MPI_MODE_RDONLY,
                           SID.file_info,
                           &(fp->fp)));
