...
size_t SID_fwrite_ordered(void *buffer, size_t size_per_item, size_t n_items, SID_fp *fp) {
    ...
    for(i_rank = 0, r_val = 0; i_rank < SID.n_proc; i_rank++) {
        if(i_rank == MASTER_RANK && SID.I_am_Master) {
            ...
            SID_Sendrecv(&n_send, 1, SID_SIZE_T, destin_rank, 1918270, &n_send, 1, SID_SIZE_T, source_rank, 1918270, SID.COMM_WORLD);
            SID_Sendrecv(&n_items_rank, 1, SID_SIZE_T, destin_rank, 1918271, &n_items_rank, 1, SID_SIZE_T, source_rank, 1918271, SID.COMM_WORLD);
            n_unsent = n_send;
            r_val_i  = 0;
            offset   = 0;
            while(n_unsent > 0) {
